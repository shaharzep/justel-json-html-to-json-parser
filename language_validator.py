...
class LanguageValidator:
    ...
    def validate_document(self, output: Dict) -> bool:
        ...
        meta_lang = output.get('metaLanguage')
        if not meta_lang:
            logger.warning("No metaLanguage specified in document")
            return False
            
        # Collect text samples for validation
        text_samples = []
        
        # Extract fullText sample (first 500 chars for better accuracy)
        ...
        for notice in notices:
            ...
            if summary and summary.strip():
                sample = summary[:200].strip()
                if sample:
                    text_samples.append(sample)
                    
            # Check keywordsFree (often contains language-specific text)
            keywords_free = notice.get('keywordsFree')
            ...
            keywords_cass = notice.get('keywordsCassation', [])
            ...
            keywords_utu = notice.get('keywordsUtu', [])
            ...
            legal_basis = notice.get('legalBasis', [])
        ...
        # Field of law
        field_of_law = output.get('fieldOfLaw')
        ...
        opinion = output.get('opinionPublicAttorney')
        if opinion and isinstance(opinion, str) and opinion.strip():
            text_samples.append(opinion[:200].strip())
            
        # Related publications fields
        rel_pub = output.get('relatedPublications', {})
        if rel_pub:
            # Decision field
            decision = rel_pub.get('decision')
            if decision and isinstance(decision, str) and decision.strip():
                text_samples.append(decision[:100].strip())
                
            # Opinion in related publications
            rel_opinion = rel_pub.get('opinionPublicAttorney')
            if rel_opinion and isinstance(rel_opinion, str) and rel_opinion.strip():
                text_samples.append(rel_opinion[:100].strip())
        
        # If no text samples found, mark as valid (no content to validate)
        if not text_samples:
            logger.info(f"No text content found for language validation in {output.get('fileName')} - marking as valid (empty document)")
            return True
        ...
        if not is_valid and llm_validator and llm_validator.is_available():
            logger.info(f"Initial validation failed for {output.get('fileName')}, using LLM validator...")
            try:
                ...
                if llm_valid and llm_confidence >= 0.8:
                    logger.info(f"LLM validation overrode initial result for {output.get('fileName')}: "
                              f"valid={llm_valid}, confidence={llm_confidence:.2f}, reason: {llm_explanation[:100]}")
                    is_valid = True
                else:
                    logger.info(f"LLM validation confirmed invalid for {output.get('fileName')}: "
                              f"confidence={llm_confidence:.2f}, reason: {llm_explanation[:100]}")
            except Exception as e:
                logger.error(f"LLM validation failed for {output.get('fileName')}: {e}")
                # Keep original validation result if LLM fails
        
        if not is_valid:
            logger.warning(f"Language validation failed for {output.get('fileName')}: "
                         f"{matches}/{total} samples matched (ratio: {match_ratio:.2f})")
        else:
            logger.debug(f"Language validation passed for {output.get('fileName')}: "
    ...
    def get_document_language_stats(self, output: Dict) -> Dict:
        ...
        """
        stats = {
            'metaLanguage': output.get('metaLanguage'),
            'detections': [],
            'isValid': False,
            'confidence': 0.0
        }
        ...
        if output.get('fullText'):
            all_text.append(output['fullText'][:1000])
            
        for notice in output.get('notices', []):
            if notice.get('summary'):
                all_text.append(notice['summary'][:500])
        ...
        if detections:
            ...
            expected_iso = self.lang_map.get(output.get('metaLanguage'))
            if expected_iso:
                for lang, prob in detections:
