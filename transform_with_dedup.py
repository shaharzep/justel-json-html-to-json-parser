...
class EnhancedJuportalTransformer(JuportalTransformer):
    ...
    def _process_full_text(self, section: Dict, output: Dict):
        ...
        from utils import extract_paragraphs_text, extract_pdf_url, clean_text
        
        # Extract all paragraph texts for plain text
        full_text = extract_paragraphs_text(section.get('paragraphs', []))
        if full_text:
            output['fullText'] = clean_text(full_text)
        
        # Extract and concatenate HTML from all paragraphs
        ...
        for para in paragraphs:
            ...
            if html and text:
                ...
                if text.startswith('Document PDF'):
                    continue
